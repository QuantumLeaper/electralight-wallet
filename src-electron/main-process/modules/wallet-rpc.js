...
export class WalletRPC {
        ...
        return new Promise((resolve, reject) => {
            ...
            crypto.randomBytes(64+64+32, (err, buffer) => {
                ...
                if (process.platform === "win32") {
                    ...
                    this.walletRPCProcess = child_process.spawn(path.join(__uplexa_bin, "uplexa-wallet-rpc"), args, {
                        detached: true
                    })
                }
    ...
    transfer (password, amount, address, payment_id, mixin, priority, address_book={}) {
        ...
        crypto.pbkdf2(password, this.auth[2], 1000, 64, "sha512", (err, password_hash) => {
            ...
            amount = parseFloat(amount).toFixed(9)*1e2
        ...
        return new Promise((resolve, reject) => {
            ...
            Promise.all([
                ...
                if(wallet.address_list.unused.length < num_unused_addresses &&
                   !wallet.address_list.primary[0].address.startsWith("UPXK") &&
                   !wallet.address_list.primary[0].address.startsWith("UPXH")) {
    ...
    listWallets(legacy=false) {
        ...
        if(legacy) {
	        ...
	        legacy_paths = ["C:\\ProgramData\\GUIWallet", "C:\\ProgramData\\LITEWallet"]
            } else {
	        legacy_paths = [path.join(os.homedir(), "GUIWallet"), path.join(os.homedir(), "LITEWallet")]
            }
