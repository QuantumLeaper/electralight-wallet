    ...
    checkVersion() {
        return new Promise((resolve, reject) => {
            if (process.platform === "win32") {
                let uplexad_path = path.join(__uplexa_bin, "uplexad.exe")
                let uplexad_version_cmd = `"${uplexad_path}" --version`
                if (!fs.existsSync(uplexad_path))
                    resolve(false)
                child_process.exec(uplexad_version_cmd, (error, stdout, stderr) => {
                    if(error)
                        resolve(false)
                    resolve(stdout)
                })
            } else {
                let uplexad_path = path.join(__uplexa_bin, "uplexad")
                let uplexad_version_cmd = `"${uplexad_path}" --version`
                if (!fs.existsSync(uplexad_path))
                    resolve(false)
                child_process.exec(uplexad_version_cmd, {detached: true}, (error, stdout, stderr) => {
                    if(error)
                        resolve(false)
                    resolve(stdout)
                })
    ...
    start(options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(options.app.testnet) {
                this.testnet = true
                args.push("--testnet")
                args.push("--log-file", path.join(options.app.data_dir, "testnet", "logs", "uplexad.log"))
            } else {
                args.push("--log-file", path.join(options.app.data_dir, "logs", "uplexad.log"))
            }
            ...
            if (process.platform === "win32") {
                this.daemonProcess = child_process.spawn(path.join(__uplexa_bin, "uplexad.exe"), args)
            } else {
                this.daemonProcess = child_process.spawn(path.join(__uplexa_bin, "uplexad"), args, {
                    detached: true
                })
            }
